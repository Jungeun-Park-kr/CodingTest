...
bool is_belt_empty(int belt_num) {
    return (belt[belt_num].tail->prev == belt[belt_num].head);
}
...
int q_200() {
    ...
    for(int i=0; i<m; ++i) {
        ...
        if(belt[i].head->next->weight <= w_max) { // 하차
            BOX* item = belt[i].head->next;
            BOX* next = belt[i].head->next->next;
            BOX* prev = belt[i].head;

            belt[i].head->next = next;
            next->prev = belt[i].head;
            ...
            item->prev = belt[i].tail->prev;
            belt[i].tail->prev->next = item;
            belt[i].tail->prev = item;
            item->next = belt[i].tail;
        }
    }
    return ret;
}
...
int q_300() { // 물건 제거
    ...
    if(it != box_map.end()) { // 꺼내기 -> 뒤에있는 박스 한칸씩 옮기기
        
        int b_num = it->second->belt; // 벨트 번호
        //printf("꺼내야 함, 벨트 %d\n", b_num);
        BOX* item = belt[b_num].head->next;
        while(item->id != r_id && item->next != belt[b_num].tail) {
            item = item->next;
        }
        //printf("찾았다 id : %d, weight : %d\n", item->id, item->weight);
        BOX* prev = item->prev;
        BOX* next = item->next;
        prev->next = next;
        next->prev = prev;
...
int main() {
    ...
    for(int tc=0; tc<q; tc++) {
        ...
        switch(cmd) {
            ...
            break;
            case 200:
                //printf(" ## 200 하차, 맨앞 상자 작으면 빼고 무게 더 크면 맨 뒤로\n");
                printf("%d\n", q_200());
            break;
            case 300:
                //printf(" ## 300 물건 제거\n");
